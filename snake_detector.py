...
def predict_species(model, class_names, image):
    """Return the predicted snake species and prediction confidence."""
...
def load_model():
    """
    Load the trained TensorFlow model and return it along with its class names.
    Ensure 'snake_detector.h5' is in the same directory.
    """
    model = tf.keras.models.load_model("snake_detector.h5")
...
# Load the model and class names (cached for performance)
...
# -------------------------
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("Predicting..."):
            species, confidence = predict_species(model, class_names, image)
